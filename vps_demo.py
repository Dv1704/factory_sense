...
def run_demo():
    print_step("Starting End-to-End Pilot Demonstration on VPS")

    # 1. Register User (Admin)
    resp = requests.post(f"{BASE_URL}/auth/register", json={"email": "vps_demo@factory.com", "password": "demo_password", "mill_id": "VPS_MILL_1", "role": "admin"})
    ...
    # 2. Login
    resp = requests.post(f"{BASE_URL}/auth/login", data={"username": "vps_demo@factory.com", "password": "demo_password"})
    if resp.status_code != 200:
    ...
    resp = requests.post(f"{BASE_URL}/admin/mills", json={"user_id": 1, "mill_id": "VPS_MILL_1"}, headers=headers)
    ...
    for i in range(100):
        baseline_csv += f"2026-03-01T10:{i%60}:00Z,VPS_MILL_1,M1,{10.0 + (i%5)*0.1},RUNNING\n"
    ...
    for i in range(60):
        ...
        op_csv += f"2026-03-02T10:{i%60}:00Z,VPS_MILL_1,M1,{12.5 + (i%5)*0.1},RUNNING\n"
    ...
    resp = requests.get(f"{BASE_URL}/dashboard/summary/VPS_MILL_1", headers=mill_headers)
    ...
    print_step("Demonstration Complete on VPS!")
